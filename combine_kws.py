...
def combine_kws(kws_path, output_path, gamma):
    ...
    for detected_kwlist in kwslist:
        sum_of_scores = sum([
            float(hit.get("score"))**gamma for hit in detected_kwlist])
        for hit in detected_kwlist:
            hit.set("score",
                    str((float(hit.get("score"))**gamma) / sum_of_scores))
...
if __name__ == '__main__':
    ...
    tree = combine_kws(args.kws_path, args.output_path, args.gamms)
